...
- `sidecar/requirements.txt` — add all missing direct dependencies with version pins: `inter-agent-trust-protocol==0.5.0`, `agent-os-kernel==3.0.1`, `agentmesh-runtime==3.0.2`, `PyJWT==2.10.1`, `httpx==0.28.1`, plus pin existing deps: `fastapi==0.132.0`, `uvicorn==0.41.0`, `cryptography==46.0.7`. Verify exact versions and imports by running `pip show <package>` in the current sidecar venv before pinning — if `server.py` does not import from `agentmesh-runtime`, remove it.
- `sidecar/requirements-lock.txt` — generated via `pip-compile requirements.txt`, full transitive dependency tree
- Modify `packages/rules-engine/src/seed.ts` — make seed function idempotent via `.onConflictDoNothing()` (Drizzle syntax) on all insert calls, or wrap `seed()` to accept an `{ onConflict: "ignore" }` option. The current `seed()` uses `db.insert().values()` which throws on duplicate primary keys — must not throw when called twice.
- Verify route paths: confirm `apps/api/src/index.ts` mounts match `apps/dashboard/src/lib/api.ts` fetch paths. Note: the API health check at `/health` conflicts with the Caddy catch-all route to the dashboard — the Caddyfile must include an explicit `/health` rule, or move the health check to `/api/health` (code change in `apps/api/src/index.ts`). The spec's Caddyfile includes the explicit rule.

**Dependencies:** None (first phase).

**Demo checkpoint:** `bun run dev` in dashboard still works (proxied via rewrites). `bun run build` in dashboard produces `.next/standalone/` directory. Sidecar starts with `uvicorn sidecar.server:app`. `pip install -r sidecar/requirements-lock.txt` in a fresh venv succeeds. `seed(db)` can be called twice in sequence without throwing (idempotent).
...
- `sidecar/Dockerfile` — Python 3.12-slim base. Build context is repo root. Preserves package structure: `COPY sidecar/__init__.py sidecar/__init__.py`, `COPY sidecar/server.py sidecar/server.py`, `COPY sidecar/requirements-lock.txt ./requirements-lock.txt`. Installs from lockfile. CMD uses `uvicorn sidecar.server:app`. Env vars: `ED25519_SEED`, `RULES_ENGINE_URL`, `PORT`. Non-root user.
  ```
...
- `drizzle/migrations/` — SQL migration files generated by `drizzle-kit generate`, committed to repo. **Prerequisite:** run `bunx drizzle-kit generate` locally and commit the output before building the API Docker image — the Dockerfile COPYs this directory at build time and will fail if it doesn't exist.

- `apps/dashboard/Dockerfile` — Multi-stage build, context is repo root. Builder stage uses `oven/bun:1.3` (matches project's package manager — `bun.lock`, not `package-lock.json`): `bun install --frozen-lockfile && bun run build` with `NEXT_PUBLIC_API_URL=__NEXT_PUBLIC_API_URL_PLACEHOLDER__`. Runner stage uses `node:20-alpine` for smallest production image, copies standalone output. Entrypoint script does `sed` replacement of placeholder with runtime `NEXT_PUBLIC_API_URL` value (if set, otherwise replaces with empty string for relative paths).
...
**Demo checkpoint:**
1. `docker build -f sidecar/Dockerfile -t warranted/governance-sidecar .` succeeds (context is repo root)
2. `docker build -f apps/api/Dockerfile -t warranted/rules-engine-api .` succeeds (context is repo root)
3. `docker build -f apps/dashboard/Dockerfile -t warranted/dashboard .` succeeds (context is repo root)
...
- `sidecar/README.md`
  ...
  - **One sidecar per agent runtime** — prominently documented as a security design choice for cryptographic isolation, not a limitation. Same pattern as Envoy, Istio, Dapr. Each sidecar has its own Ed25519 identity derived from `ED25519_SEED` — no agent can sign as another agent.
...
- `apps/dashboard/README.md`
  ...
  - Screenshots of key pages (policies list, envelope visualization, REPL tester, Cedar viewer)
...
- `.env.example` — committed to repo, documents all required and optional env vars for production compose:
  ```env
  # Required
...
- `docker-compose.production.yml` — reference production deployment:
  ...
  services:
    ...
    api:
      ...
      ports: 3000:3000  # Remove in production when using Caddy proxy profile
