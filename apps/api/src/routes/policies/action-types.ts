...
import { actionTypes, dimensionDefinitions } from "@warranted/rules-engine";
import type { DrizzleDB } from "@warranted/rules-engine";

export function actionTypesRoutes(db: DrizzleDB): Hono {
  const app = new Hono();

  // GET / — List all action types with dimensions
  app.get("/", async (c) => {
    const types = await db.select().from(actionTypes);
    const dims = await db.select().from(dimensionDefinitions);
  ...
  // GET /:id — Get action type with dimension definitions
  app.get("/:id", async (c) => {
    const id = c.req.param("id");
    const typeRows = await db.select().from(actionTypes).where(eq(actionTypes.id, id));
